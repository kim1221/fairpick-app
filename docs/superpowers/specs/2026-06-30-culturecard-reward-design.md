...
| 차별화 레이어 | **문화 여권 + "가봤어요" 도장** — 포인트 옆 문화적 이점층 (수집 대상=돈이 아니라 문화 경험) |
...
"가봤어요" 도장 보너스 **+3 티켓**(추후 튜닝 가능). 04 단독 "포인트" 화면은 폐기, 05 "내 문화"가 상위호환.
...
## 2.5 차별화 레이어 — 문화 여권 + "가봤어요" 도장 (이 앱의 "이점" 정체성)
...
- **"가봤어요" 도장**: 저장한 행사를 실제로 방문하고 체크인하면 여권에 도장 + **보너스 티켓**(서버 권위·멱등).
  디지털 발견 → 실제 문화생활로 잇는 가장 강한 실질 이점. ("이 앱이 나를 밖으로 나가게 했다")
...
- 각 행사에 **"가봤어요" 체크인** → 여권 도장 + 보너스 티켓 (실제 방문 유도)
...
- **하단 = 최근 내역**(history API): 카드 열기 +n · 출석 +1 · 가봤어요 도장 +n · 토스포인트 교환 −10
...
- **가봤어요 체크인(신규)**: `POST /api/visits { eventId }` — 행사당 1회 멱등, 여권 도장 +
  보너스 티켓 서버 권위 지급(earn_log/장부에 'visit' 사유로 기록). daily/abuse 가드.
...
3. **구현(codex 위임 + 검수)**:
   - 백엔드: `GET /api/cards/today` + `POST /api/visits`(가봤어요/도장+보너스, 멱등) + `GET /api/passport`(여권 통계) 추가
